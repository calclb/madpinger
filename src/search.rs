...
use reqwest::header::{ACCEPT, ACCEPT_ENCODING, ACCEPT_LANGUAGE, CONNECTION, CONTENT_LENGTH, CONTENT_TYPE, HeaderMap, HeaderValue, USER_AGENT};
...
pub fn get_payload(term_code: &str, search: &str, page_size: usize, filters: CourseStatusFilters) -> String {
...
pub async fn get_search_info(client: Client, term_code: &str, search: &str, size: usize, filters: CourseStatusFilters) -> Result<(), ReqwestError> {
    ...
    hdrs.insert(ACCEPT_ENCODING, HeaderValue::from_static("gzip, deflate, br")); // not required
    ...
    let resp = client.post(SEARCH_POST_URI_BASE)
        .headers(hdrs)
        .body(payload)
        .send()
        .await?;
    ...
    File::create(OUTPUT_FILE_NAME).unwrap().write_all(text.as_bytes()).unwrap();
    Ok(())
}
